...
prompt_virtualenv() {
  if [[ -n "$VIRTUAL_ENV" && -n "$VIRTUAL_ENV_DISABLE_PROMPT" ]]; then
    local env_label="(${VIRTUAL_ENV:t:gs/%/%%})"
  fi
...
prompt_brazil() {
  # Test if brazil workspace in directory hierarchy
  local dir="$PWD"
  while [[ ! -f "$dir/packageInfo" ]]; do
    [[ "$dir" = "$HOME/workplace" ]] && return
    [[ "$dir" = "$HOME" ]] && return
    [[ "$dir" = "/" ]] && return
    dir="${dir:h}"
  done

  # todo: create a cache file of the JSON output, hash original file for speed comparisons
  brazil_status=$(brazil ws show -f json 2>&1)
  status_name=$(echo -n "$brazil_status" | jq -r ".name")
  prompt_segment green black "\uf215 $status_name"

  #status_name=$(_getvar workspace "$dir/packageInfo")
  #prompt_segment green black "\uf215 $status_name"

  #if brazil_status=$(brazil ws show -f json 2>&1); then
  #  status_name=$(echo -n "$brazil_status" | jq -r ".name")
  #  prompt_segment green black "\uf215 $status_name"
  #fi
}
